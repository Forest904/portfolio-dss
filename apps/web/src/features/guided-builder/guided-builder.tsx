...
export function GuidedBuilder({ active = true }: { active?: boolean }) {
  ...
  function invalidate() {
    ...
    setSubmitting(false); setJobId(null); setReport(null); setError(null);
  }

  async function submit(event?: FormEvent) {
    event?.preventDefault();
    if (!complete || !/^\d+(\.\d{1,2})?$/.test(capital) || !Number.isFinite(Number(capital)) || Number(capital) <= 0) {
      setError("Enter positive USD capital with at most two decimal places."); return;
    }
    invalidate();
    const version = revision.current;
    const controller = new AbortController();
  ...
  return <section className="analysis-workspace guided-builder" aria-labelledby="builder-title">
    ...
    {step === 0 && <form className="portfolio-form" onSubmit={(e) => { e.preventDefault(); if (complete) setStep(1); }}>
      {questions.map((q) => <fieldset className="profile-controls" key={q.id}><legend>{q.title}</legend>{q.choices.map((choice, i) => <label className={answers[q.id] === profiles[i] ? "profile-option selected" : "profile-option"} key={choice}>
